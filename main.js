...
/* ---------------------------------------------------------------------
   Piazzamento pezzi: drag basato su Pointer Events (non sul Drag&Drop HTML5,
   che i browser NON attivano affatto con input touch). Un pointerdown sul
   pezzo crea un "fantasma" che segue il dito/mouse; al rilascio, se si è
   sopra l'area di gioco, il pezzo viene piazzato. Stesso codice per
   mouse, touch e penna: i Pointer Event li unificano tutti.
   --------------------------------------------------------------------- */
const stageWrap = el('#stage-wrap');
let activeDrag = null; // { type, ghost, pointerId }
...
document.querySelectorAll('.piece').forEach(piece => {
  ...
  piece.addEventListener('pointerdown', ev => {
    ...
    activeDrag = { type, ghost, pointerId: ev.pointerId };
  });
});

document.addEventListener('pointermove', ev => {
  if (!activeDrag || ev.pointerId !== activeDrag.pointerId) return;
  activeDrag.ghost.style.left = ev.clientX + 'px';
  activeDrag.ghost.style.top = ev.clientY + 'px';
...
function endDrag (ev, commit) {
  if (!activeDrag || (ev && ev.pointerId !== activeDrag.pointerId)) return;
  const { type, ghost } = activeDrag;
  ghost.remove();
  stageWrap.classList.remove('drag-over');
  if (window.__scene) window.__scene.clearDropPreview();

  if (commit && ev && window.__scene) {
    const { over } = stagePointFromClient(ev.clientX, ev.clientY);
    if (over) {
