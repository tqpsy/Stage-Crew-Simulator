...
const COMPONENT_TYPES = {
  sub: {
    ...
    ports: [
      { id: 'power',    signal: 'powercon', dx: 0,   dy: 27 },
      { id: 'spk_in',   signal: 'speakon',  dx: -27, dy: 2 },
      { id: 'spk_thru', signal: 'speakon',  dx: 0,   dy: -27 }
    ]
  },
  top: {
    ...
    ports: [
      { id: 'spk_in', signal: 'speakon', dx: 0, dy: 22 }
    ]
  },
  mixer: {
    ...
    ports: [
      { id: 'power',   signal: 'powercon', dx: -27, dy: -19 },
      { id: 'audio_L', signal: 'xlr',      dx: -14, dy: 21 },
      { id: 'audio_R', signal: 'xlr',      dx: 14,  dy: 21 }
    ]
  },
  ampli: {
    ...
    ports: [
      { id: 'power', signal: 'powercon', dx: -32, dy: -18 },
      { id: 'in_L',  signal: 'xlr',      dx: -30, dy: 18 },
      { id: 'in_R',  signal: 'xlr',      dx: -10, dy: 18 },
      { id: 'out_L', signal: 'speakon',  dx: 12,  dy: 18 },
      { id: 'out_R', signal: 'speakon',  dx: 32,  dy: 18 }
    ]
  },
  par: {
    ...
    ports: [
      { id: 'power_in',   signal: 'powercon', dx: -16, dy: 20 },
      { id: 'power_thru', signal: 'powercon', dx: 16,  dy: 20 },
      { id: 'dmx_in',     signal: 'dmx',      dx: -16, dy: -16 },
      { id: 'dmx_thru',   signal: 'dmx',      dx: 16,  dy: -16 }
    ]
  },
  controller: {
    ...
    ports: [
      { id: 'power', signal: 'powercon', dx: -24, dy: 22 },
      { id: 'dmx',   signal: 'dmx',      dx: 24,  dy: 22 }
    ]
  },
  quadro: {
    ...
    ports: [
      { id: 'out', signal: 'powercon', dx: 0, dy: -29 }
    ]
  }
};
...
function runValidation () {
  ...
  return { pass: allFound && !overBudget, failedComponents, overBudget, usedW };
}
...
/* Drag start/end for toolbar pieces (traccia il tipo trascinato, usato per l'anteprima) */
document.querySelectorAll('.piece').forEach(piece => {
  piece.addEventListener('dragstart', ev => {
    if (piece.classList.contains('depleted')) { ev.preventDefault(); return; }
    window.__draggedType = piece.dataset.type;
    ev.dataTransfer.setData('text/plain', piece.dataset.type);
    ev.dataTransfer.effectAllowed = 'copy';
  });
  piece.addEventListener('dragend', () => { window.__draggedType = null; });
});

/* Drop onto stage */
const stageWrap = el('#stage-wrap');
stageWrap.addEventListener('dragover', ev => {
  ev.preventDefault();
  ev.dataTransfer.dropEffect = 'copy';
  stageWrap.classList.add('drag-over');
  if (window.__scene) window.__scene.previewDropCell(ev.clientX, ev.clientY);
});
...
stageWrap.addEventListener('drop', ev => {
  ev.preventDefault();
  stageWrap.classList.remove('drag-over');
  const type = ev.dataTransfer.getData('text/plain');
  if (!type || !window.__scene) return;
  if (gameState.stock[type] <= 0) { showToast(type.toUpperCase() + ' esaurito per questo livello.'); return; }
  window.__scene.handleExternalDrop(type, ev.clientX, ev.clientY);
});
...
class StageScene extends Phaser.Scene {
  ...
  handleExternalDrop (type, clientX, clientY) {
    ...
    if (type === 'top') { this.attachTopToNearestSub(world); return; }

    const { cx, cy } = this.nearestAllowedCell(type, world.x, world.y);
    const key = cx + ',' + cy;
  ...
  handlePortClick (componentId, portId, signal) {
    ...
    gameState.edges.push({
      ...
      a: pending.componentId, aPort: pending.portId,
      b: componentId, bPort: portId,
